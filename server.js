...
//Set accountList to service.js
service.accountList = JSON.parse(data);
//First user login by default
//service.accountList[0].onSeat = true;
service.initAccountList(data);
	...
	if(service.isInList(userName)){
		...
		for (i in service.accountList){
			if (service.accountList[i].name == userName){
				service.accountList[i].onSeat = true;
			}
		}
	}
...
app.post('/getCurrentList', function (req, res) {
	...
	res.json({userList:service.userList,result:service.result,unit:service.unit});
});
